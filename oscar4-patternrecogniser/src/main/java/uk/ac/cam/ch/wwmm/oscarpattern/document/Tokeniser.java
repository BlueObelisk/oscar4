...
public final class Tokeniser {
	...
	private static final Pattern TRADEMARK_PATTERN = Pattern.compile(".+?(\\((TM|R)\\)|\\(\\((TM|R)\\)\\))");

	private static final Pattern TOKEN_PATTERN = Pattern.compile("[^\\s" + StringTools.whiteSpace + "]+");
	private static final Pattern CONSECUTIVE_LOWERCASE_PATTERN = Pattern.compile("[a-z][a-z]");
	private static final Pattern SINGLE_UPPERCASE_PATTERN = Pattern.compile("[A-Z]");
	
	
	private static final Pattern NAMED_REAGENT_PATTERN = Pattern.compile("[A-Z][a-z]+s'");
	...
	private static final Pattern prefixPattern = Pattern.compile(prefixRe + 
	...
	private List<Token> rawSplitToken(Token token) {
		...
		if(OXIDATION_STATE_PATTERN.matcher(token.value).matches()) {
			return null;
		...
		if(OXIDATION_STATE_END_PATTERN.matcher(token.value).matches()) {
		...
		if((".,;:!?\u2122\u00ae-" + StringTools.quoteMarks).indexOf(token.value.codePointAt(token.value.length()-1)) != -1) {
			...
			if(!(token.value.substring(token.value.length()-1).equals("'") && NAMED_REAGENT_PATTERN.matcher(token.value).matches())) {
				return splitAt(token, token.end-1);				
			}
		}
		Matcher m = TRADEMARK_PATTERN.matcher(token.value);
		...
		if(splittableHyphenIndex != -1 && !CONSECUTIVE_LOWERCASE_PATTERN.matcher(token.value).find() && SINGLE_UPPERCASE_PATTERN.matcher(token.value).find()) {
	...
	public void tidyHyphensAfterNEs(List<Token> tokens) {
		int i=0;
