...
public class Oscar4RegressionTest {
	...
	public void testSentence1RemoveBlocked() throws Exception {
		...
		List<String> expectedSurfaceList = Arrays.asList(
				...
				"malononitrile_2");
		List<Double> expectedProbList = Arrays.asList(0.9362979800888434,
				0.9956194686746215, 0.30735471308029166, 0.00,
				0.9999484864253582, 0.34065781326024086,
				0.9400554713275171, 0.9956696660305651);
	...
	public void testSentence3RemoveBlocked() throws Exception {
		...
		List<Double> expectedProbList = Arrays.asList(0.39248159697863777, 0.00, 0.00,
				0.968218689388447, 0.5126968674819777, 0.00, 0.00, 0.46708681080060027,
	...
	public void testSentence6RemoveBlocked() throws Exception {
		...
		List<String> expectedSurfaceList = Arrays.asList(
				"8-hydroxyquinoline-7-carboxylic acid_1", "benzylamine_1",
				"1\u2010hydroxybenzotriazole_1",
				"1-(3-dimethylaminopropyl)-3-ethylcarbodiimide hydrochloride_1",
				"triethylamine_1", "DMF_1", "methanol_1",
				"C-18_1", "water_1", "acetonitrile_1", "TFA_1");
		List<Double> expectedProbList = Arrays.asList(0.9763876481410927,
				...
				0.768896911785475);
		List<String> expectedTypeList = Arrays.asList("CM", "CM", "CM", "CM", "CM",
				"CM", "CM", "CM", "CM", "CM", "CM");
		evaluateNamedEntities(sentence, ResolutionMode.REMOVE_BLOCKED,
				expectedSurfaceList, expectedTypeList, expectedProbList);
	}
	...
	public void testSentence6MarkBlocked() throws Exception {
		...
		List<Double> expectedProbList = Arrays.asList(0.00, 0.9763876481410927, 0.00, 0.877166645595111, 0.00, 0.8070202675168012, 0.00, 0.8771168559135208, 0.00, 0.9430003382163082, 0.7821893198686901, 0.9943684604393002, 0.00, 0.39805870405590443, 0.9726097731082048, 0.9469694587383923, 0.768896911785475);
