...
public final class HyphenTokeniser {
	...
	private final Logger logger = Logger.getLogger(HyphenTokeniser.class);
	...
	private int indexOfSplittableHyphenInternal(String s) {
		...
		for(int i=s.length()-3;i>0;i--) {
			...
			if(StringTools.hyphens.indexOf(s.codePointAt(i)) != -1) {
				...
				Pattern suffHyphPattern = Pattern.compile("([A-Z][0-9][A-Z][0-9])+");
				Matcher suffMatch = suffHyphPattern.matcher(s);
				if(suffMatch.find()) {
					return i;
				}
