...
public final class ProcessingDocument {
	...
	/**Gets the token that starts at a given XPoint. Note that this should
	 * only be called after the document has been tokenised.
	 * 
	 * @param leftXPoint The XPoint of the start of the token.
	 * @return The token, or null.
	 * @throws Exception
	 */
	public Token getTokenByStart(String leftXPoint) throws Exception {
		int offset = standoffTable.getOffsetAtXPoint(leftXPoint);
		if(tokensByStart.containsKey(offset)) return tokensByStart.get(offset);
		return null;
	}

	/**Gets the token that ends at a given XPoint. Note that this should
	 * only be called after the document has been tokenised.
	 * 
	 * @param rightXPoint The XPoint of the end of the token.
	 * @return The token, or null.
	 * @throws Exception
	 */
	public Token getTokenByEnd(String rightXPoint) throws Exception {
		int offset = standoffTable.getOffsetAtXPoint(rightXPoint);
		if(tokensByEnd.containsKey(offset)) return tokensByEnd.get(offset);
		return null;
	}
